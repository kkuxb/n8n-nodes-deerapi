# n8n-nodes-DeerAPI（V1.6.0 版本）

---

> ## ⚠️ 重要公告：项目已停止维护，请迁移至新项目
> 
> 由于合作的模型服务商变更，**本项目（n8n-nodes-deerapi）已停止维护。**
> 
> **当前合作服务商已更换为 [麦包API](https://api.maibao.chat)**。继续使用本节点，需前往麦包API官网注册账号并获取 API 密钥。**如想要继续使用DeerAPI这个服务商，请勿升级至1.6.0版本！**
> 
> ---
> 
> ### 🎉 新项目已上线，功能更强大！
> 
> 本项目已正式迁移至 **[n8n-nodes-maibaoapi](https://www.npmjs.com/package/n8n-nodes-maibaoapi)**，后续所有功能迭代和维护均在新项目进行。
> 
> 新项目已新增支持：
> 
> - **Nano banana 系列新模型**
...
🚀 **告别见鬼的 HTTP Request 节点！** 在 n8n 中像原生节点一样傻瓜式调用麦包API中的模型。
...
由于官方目前只做了 Dify 的节点，完全忽视了我们 n8n 用户的感受。因此，本项目诞生了！
...
- **生图模型**：当前支持以下模型：
  ...
  | ------------------- | ---------------------------- | --------------- | ------ |
  | **Nano banana Pro** | `gemini-3-pro-image`         | ✅ 支持 (1K/2K/4K) | ✅ 支持   |
  | **Nano banana**     | `gemini-2.5-flash-image`     | ❌ 不需要           | ✅ 支持   |
  | **即梦 4.5**          | `doubao-seedream-4-5-251128` | ✅ 支持 (2K/4K)    | ❌ 不支持  |
...
- **新增模型**：图像生成模式新增 `Gemini-2.5-Flash-Image` 模型
...
1. **添加凭证：** 在 n8n 凭证管理页面创建凭证，凭证搜索"DeerAPI"。凭证只需要填入从 [麦包API](https://api.maibao.chat) 后台获取的 API Key 就行了。

2. **添加节点：** 在工作流中添加节点时，可搜索"DeerAPI"，即可找到本节点（找不到是因为你还没安装本社区节点）

3. **节点配置：** 选择凭证 → 选择模式（文本生成/图像生成）→ 填好其它两三个参数（模型 ID、提示词等）→ OJ8K 完事！
